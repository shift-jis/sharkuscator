...
class Sharkuscator(private val configurationFilePath: Path, private val inputJarFile: File, private val outputJarFile: File) {
    ...
    fun obfuscate() {
        ...
        inputJarContents = readJarFileContents(inputJarFile, true)
        ...
        for (libraryFilePath in configuration.libraries) {
            classSource.addLibraries(readLibrarySource(classSource, File(libraryFilePath)))
        }
    ...
    private fun importConfiguration(): GsonConfiguration {
        try {
            ...
            return ObfuscatorServices.jsonProcessor.fromJson(configurationFilePath.readText(), GsonConfiguration::class.java)
    ...
    private fun readLibrarySource(classSource: ApplicationClassSource, libraryFile: File): LibraryClassSource {
        return LibraryClassSource(classSource, readJarFileContents(libraryFile, false).classContents)
    }

    @Throws(IOException::class)
    private fun readJarFileContents(jarFile: File, usePhantomFactory: Boolean): JarContents<ClassNode> {
        val asmFactory: ASMFactory<ClassNode> = if (usePhantomFactory) {
            createPhantomAsmFactory(jarFile)
        } else {
            DefaultASMFactory()
        }
        return PhantomJarDownloader(asmFactory, JarInfo(jarFile)).apply { download() }.jarContents
    }
