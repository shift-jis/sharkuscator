...
object FieldRenameTransformer : BaseTransformer<RenameConfiguration>("FieldRename", RenameConfiguration::class.java) {
    ...
    private fun onFieldTransform(event: TransformerEvents.FieldTransformEvent) {
        if (!isEligibleForExecution() || !shouldTransformField(event.context, event.anytypeNode) || disallowedInterfacePatterns.any { it.matches(event.anytypeNode.owner.node.superName) }) {
            return
        }

        val targetClassNode = event.anytypeNode.owner
        if (event.context.classSource.isLibraryClass(targetClassNode.name) || targetClassNode.isAutoGenerated() || targetClassNode.isEnum) {
            return
        }
