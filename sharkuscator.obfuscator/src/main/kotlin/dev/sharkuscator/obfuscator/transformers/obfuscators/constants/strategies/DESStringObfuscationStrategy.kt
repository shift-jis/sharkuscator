...
class DESStringObfuscationStrategy : StringConstantObfuscationStrategy {
    ...
    override fun finalizeClass(context: ObfuscationContext, targetClassNode: ClassNode) {
        ...
        keyByteIndexChunks.forEach { chunkOfIndices ->
            ...
            hostClassNode.addField(BytecodeUtils.createFieldNode(KEY_BYTES_FIELD_ACCESS, generatedKeyFieldName, KEY_BYTES_FIELD_DESCRIPTOR))
            classToKeyChunkList.add(Pair(targetClassNode, EncryptionKeyChunk(hostClassNode, generatedKeyFieldName, chunkOfIndices)))
        }
        ...
        initializerInstructions.add(BytecodeUtils.integerPushInstruction(currentClassEncryptedStrings.size))
        ...
        initializerInstructions.add(BytecodeUtils.integerPushInstruction(decryptionKeyBytes.size))
        ...
        for ((_, keyChunk) in classToKeyChunkList.filter { it.first == targetClassNode }) {
            ...
            initializerInstructions.add(BytecodeUtils.integerPushInstruction(0))
            initializerInstructions.add(VarInsnNode(Opcodes.ALOAD, 3))
            initializerInstructions.add(BytecodeUtils.complexIntegerPushInstruction(keySegmentOffset))
            initializerInstructions.add(FieldInsnNode(Opcodes.GETSTATIC, keyChunk.classNode.name, keyChunk.fieldName, KEY_BYTES_FIELD_DESCRIPTOR))
        ...
        initializerInstructions.add(InsnNode(Opcodes.DUP))
        initializerInstructions.add(BytecodeUtils.integerPushInstruction(8))
        ...
        initializerInstructions.add(BytecodeUtils.integerPushInstruction(currentClassEncryptedStrings[0].length))
        initializerInstructions.add(VarInsnNode(Opcodes.ISTORE, 6))
        initializerInstructions.add(BytecodeUtils.integerPushInstruction(-1))
    ...
    fun initializeKeyChunkFields(targetClassNode: ClassNode) {
        ...
        for ((_, keyChunk) in classToKeyChunkList.filter { it.first == targetClassNode }) {
            ...
            replacementInstructions.add(BytecodeUtils.longPushInstruction(keyDerivationSeed))
            ...
            replacementInstructions.add(BytecodeUtils.integerPushInstruction(keyChunk.chunkOfIndices.size))
            ...
            replacementInstructions.add(VarInsnNode(Opcodes.ILOAD, localVarBaseIndex + 3))
            replacementInstructions.add(BytecodeUtils.complexIntegerPushInstruction(chunkByteOffset))
            ...
            replacementInstructions.add(BytecodeUtils.integerPushInstruction(56))
