...
class DESStringObfuscationStrategy : StringConstantObfuscationStrategy {
    ...
    override fun finalizeClass(context: ObfuscationContext, targetClassNode: ClassNode) {
        ...
        keyByteIndexChunks.forEach { chunkOfIndices ->
            val suitableClassNode = context.jarContents.classContents.filter {
                !it.isDeclaredAsAnnotation() && !it.isDeclaredAsInterface() && !it.isAutoGenerated() && !it.isSpongeMixin() && !it.isAnonymousClass() &&
                        !context.exclusions.excluded(it) && !it.isEnum
            }.random() ?: return@forEach

            val generatedKeyFieldName = fieldDictionary.generateNextName(null)
            suitableClassNode.addField(BytecodeUtils.createFieldNode(KEY_BYTES_FIELD_ACCESS, generatedKeyFieldName, KEY_BYTES_FIELD_DESCRIPTOR))
            classToKeyChunkList.add(Pair(targetClassNode, EncryptionKeyChunk(suitableClassNode, generatedKeyFieldName, chunkOfIndices)))
        }
        ...
        for ((_, keyChunk) in classToKeyChunkList.filter { it.first == targetClassNode }) {
            replacementInstructions.add(FieldInsnNode(Opcodes.GETSTATIC, keyChunk.classNode.name, keyChunk.fieldName, KEY_BYTES_FIELD_DESCRIPTOR))
            ...
            replacementInstructions.add(BytecodeUtils.complexIntegerPushInstruction(keySegmentOffset))
            replacementInstructions.add(FieldInsnNode(Opcodes.GETSTATIC, keyChunk.classNode.name, keyChunk.fieldName, KEY_BYTES_FIELD_DESCRIPTOR))
    ...
    fun initializeKeyChunkFields(targetClassNode: ClassNode) {
        ...
        for ((_, keyChunk) in classToKeyChunkList.filter { it.first == targetClassNode }) {
            ...
            replacementInstructions.add(IntInsnNode(Opcodes.NEWARRAY, Opcodes.T_BYTE))
            for ((index, originalIndex) in keyChunk.chunkOfIndices.withIndex()) {
                replacementInstructions.add(InsnNode(Opcodes.DUP))
                replacementInstructions.add(BytecodeUtils.complexIntegerPushInstruction(index))
                replacementInstructions.add(BytecodeUtils.complexIntegerPushInstruction(decryptionKeyBytes[originalIndex]))
            ...
            staticInitializer.node.instructions.insertBefore(staticInitializer.node.instructions.first, replacementInstructions)
        }
    }
}
