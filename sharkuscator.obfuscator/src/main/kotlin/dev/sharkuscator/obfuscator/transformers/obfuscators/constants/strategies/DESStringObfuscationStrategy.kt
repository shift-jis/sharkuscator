...
class DESStringObfuscationStrategy : StringConstantObfuscationStrategy {
    ...
    override fun finalizeClass(context: ObfuscationContext, targetClassNode: ClassNode) {
        ...
        val classToKeyFieldInfo = mutableMapOf<ClassNode, Pair<String, Int>>()

        (0..decryptionKeyBytes.size - 1).shuffled().chunked((1..decryptionKeyBytes.size - 1).random()).forEach { index ->
            val suitableClassNode = context.jarContents.classContents.filter {
                !it.isDeclaredAsAnnotation() && !it.isDeclaredAsInterface() && !it.isAutoGenerated() && !it.isSpongeMixin() && !it.isAnonymousClass() && !it.isEnum
            }.random() ?: return@forEach

            val keyBytesFieldName = fieldDictionary.generateNextName(null)
            suitableClassNode.addField(BytecodeUtils.createFieldNode(KEY_BYTES_FIELD_ACCESS, keyBytesFieldName, KEY_BYTES_FIELD_DESCRIPTOR))

            val replacementInstruction = InsnList()
            replacementInstruction.add(BytecodeUtils.integerPushInstruction(index.size))
            replacementInstruction.add(IntInsnNode(Opcodes.NEWARRAY, Opcodes.T_BYTE))
            for ((index, i) in index.withIndex()) {
                replacementInstruction.add(InsnNode(Opcodes.DUP))
                replacementInstruction.add(BytecodeUtils.integerPushInstruction(index))
                replacementInstruction.add(BytecodeUtils.integerPushInstruction(decryptionKeyBytes[i]))
                replacementInstruction.add(InsnNode(Opcodes.BASTORE))
            }
            replacementInstruction.add(FieldInsnNode(Opcodes.PUTSTATIC, suitableClassNode.name, keyBytesFieldName, KEY_BYTES_FIELD_DESCRIPTOR))

            val staticInitializer = suitableClassNode.getOrCreateStaticInitializer()
            staticInitializer.node.instructions.insertBefore(staticInitializer.node.instructions.first, replacementInstruction)
            classToKeyFieldInfo[suitableClassNode] = Pair(keyBytesFieldName, classToKeyFieldInfo.getOrPut(suitableClassNode) { Pair(keyBytesFieldName, 0) }.second + index.size)
        }
        ...
        var currentCount = 0
        for ((classNode, keyFieldInfo) in classToKeyFieldInfo) {
            replacementInstructions.add(FieldInsnNode(Opcodes.GETSTATIC, classNode.name, keyFieldInfo.first, KEY_BYTES_FIELD_DESCRIPTOR))
            ...
            replacementInstructions.add(BytecodeUtils.integerPushInstruction(currentCount))
            replacementInstructions.add(FieldInsnNode(Opcodes.GETSTATIC, classNode.name, keyFieldInfo.first, KEY_BYTES_FIELD_DESCRIPTOR))
            replacementInstructions.add(InsnNode(Opcodes.ARRAYLENGTH))
            replacementInstructions.add(MethodInsnNode(Opcodes.INVOKESTATIC, "java/lang/System", "arraycopy", "(Ljava/lang/Object;ILjava/lang/Object;II)V"))
            currentCount += keyFieldInfo.second
        }
        ...
        val returnInstruction = staticInitializer.node.instructions.find { it.opcode == Opcodes.RETURN } ?: return
        staticInitializer.node.instructions.insertBefore(returnInstruction, replacementInstructions)
    }
